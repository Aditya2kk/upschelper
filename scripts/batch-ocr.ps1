...
try {
    ...
    $imageFiles = Get-ChildItem -Path $inputDir -Filter "*.png" | Sort-Object Name
    ...
    foreach ($img in $imageFiles) {
        try {
            ...
            $outPath = Join-Path $outputDir ($img.BaseName + ".txt")
            [System.IO.File]::WriteAllText($outPath, $result.Text, [System.Text.Encoding]::UTF8)
        } catch {
            $errPath = Join-Path $outputDir ($img.BaseName + ".txt")
            [System.IO.File]::WriteAllText($errPath, "", [System.Text.Encoding]::UTF8)
        }
    }

    [Console]::WriteLine("BATCH_OCR_SUCCESS:" + $imageFiles.Count)
    exit 0
} catch {
